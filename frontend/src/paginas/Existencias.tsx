...
export default function Existencias() {
  ...
  // Modal para ver lotes y vencimientos detallados por producto
  const [modalLotesProducto, setModalLotesProducto] = useState<{ id: number; nombre: string; sku: string; unidad: string } | null>(null)
  ...
  function abrirModalLotes(productoId: number, nombre: string, sku: string, unidad: string) {
    setModalLotesProducto({ id: productoId, nombre, sku, unidad })
    setCargandoLotes(true)
    const params = new URLSearchParams()
    params.set('productoId', String(productoId))
    if (almacenId) params.set('almacenId', almacenId)

    api<LoteDetalle[]>(`/api/existencias/lotes?${params}`)
      .then(setLotesDetalle)
      .catch(() => setLotesDetalle([]))
  ...
  return (
    <>
                  ...
                  <tr key={e.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => abrirModalLotes(e.producto.id, e.producto.nombre, e.producto.sku, e.producto.unidadMedida)}
                          className="px-2.5 py-1 text-xs font-bold text-[#3755c3] bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-all flex items-center gap-1 cursor-pointer"
                          title="Ver desglose de lotes y fechas de vencimiento"
                        >
                        ...
                        <button
                          ...
                          className="material-symbols-outlined !text-slate-400 hover:text-[#3755c3] transition-colors cursor-pointer text-lg p-1.5 hover:bg-slate-100 rounded-lg !bg-white border border-slate-200 flex items-center justify-center"
      ...
      {/* Modal Desglose de Lotes y Vencimientos */}
      {modalLotesProducto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-5 bg-gradient-to-r from-slate-900 to-[#001f51] text-white flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-amber-400 text-2xl">event_busy</span>
                <div>
                  <h3 className="text-sm font-extrabold uppercase tracking-wider text-white">Desglose de Lotes y Vencimientos</h3>
                  <p className="text-xs text-blue-200 font-medium">{modalLotesProducto.nombre} ({modalLotesProducto.sku})</p>
                </div>
              </div>
              <button
                onClick={() => setModalLotesProducto(null)}
                className="text-slate-400 hover:text-white p-1 rounded-lg transition-colors cursor-pointer !bg-transparent"
              >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              {cargandoLotes ? (
                <div className="py-8 text-center text-slate-500">
                  <span className="material-symbols-outlined text-3xl animate-spin text-[#3755c3] mb-2 block">progress_activity</span>
                  <p className="text-xs font-bold uppercase tracking-wider">Cargando Lotes...</p>
                </div>
              ) : lotesDetalle.length > 0 ? (
                <div className="space-y-3">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-wider flex justify-between px-1">
                    <span>Lotes Activos ({lotesDetalle.length})</span>
                    <span>Total: {lotesDetalle.reduce((a, b) => a + b.cantidad, 0)} {modalLotesProducto.unidad}</span>
                  </div>

                  {lotesDetalle.map((lote) => {
                    const esVencido = lote.diasParaVencer !== null && lote.diasParaVencer <= 0
                    const esProximo = lote.diasParaVencer !== null && lote.diasParaVencer > 0 && lote.diasParaVencer <= 30
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs font-black text-[#001f51] bg-white px-2 py-0.5 rounded border border-slate-200">
                              LOTE: {lote.codigoLote}
                            </span>
                            <span className="text-[11px] font-bold text-slate-500">({lote.almacenNombre})</span>
                          </div>
                          <span className="text-sm font-black text-slate-800">
                            {lote.cantidad} <span className="text-xs text-slate-400 font-normal">{modalLotesProducto.unidad}</span>
                          </span>
                        </div>

                        <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-200/60">
                          <span className="text-slate-500 font-medium">Vencimiento: <strong>{lote.fechaVencimiento}</strong></span>
                          {lote.diasParaVencer !== null ? (
                            <span className={`font-bold px-2 py-0.5 rounded text-[10px] uppercase ${
                              ...
                              {esVencido ? '¡VENCIDO!' : `${lote.diasParaVencer} días restantes`}
                            </span>
                          ) : (
                            <span className="text-slate-400 italic text-[10px]">Sin Vencimiento</span>
                          )}
                        </div>
                      </div>
                    )
                ...
                <div className="p-8 text-center text-slate-400 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  <p className="text-xs font-bold text-slate-600 mb-1">Sin lotes registrados</p>
                  <p className="text-[11px] text-slate-400">No hay lotes con existencias para este insumo en el almacén seleccionado.</p>
                </div>
              )}
            </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end shrink-0">
              <button
                type="button"
                onClick={() => setModalLotesProducto(null)}
                className="px-5 py-2 bg-[#001f51] hover:bg-[#00337c] text-white rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
              >
                Cerrar
              </button>
            </div>
